...
exports.runResearchAgent = void 0;
...
const yahooFinance = new yahoo_finance2_1.default({ suppressNotices: ['yahooSurvey', 'ripHistorical'] });
...
function buildFallbackAnalysis(companyName, numbers, overview, researchData) {
...
function extractNewsItems(researchData) {
    if (!researchData) {
        return [
...
function parseModelOutput(content, fallback) {
    if (!content)
        return fallback;
...
async function generateFollowUpAnswer(question, companyName, result) {
    ...
    try {
        ...
        const response = await model.invoke([
            new messages_1.SystemMessage('You are a concise investment research assistant. Answer the user question using the provided research context. Keep the reply brief, practical, and grounded in the data.'),
            new messages_1.HumanMessage(`Company: ${name}\nQuestion: ${question}\nResearch Summary: ${result?.reasoning || 'No summary available.'}\nRisk: ${result?.risk || 'Unknown'}\nGrowth: ${result?.growth || 'Unknown'}\nNumbers: ${JSON.stringify(result?.numbers || {})}`),
        ]);
...
async function answerFollowUpQuestion(question, companyName, result) {
...
const StateAnnotation = langgraph_1.Annotation.Root({
    ...
    researchData: (0, langgraph_1.Annotation)(),
...
const researchNode = async (state) => {
    ...
    const tool = new duckduckgo_search_1.DuckDuckGoSearch({ maxResults: 5 });
...
async function fetchAlphaVantage(ticker) {
    ...
    if (!series)
        throw new Error('Alpha Vantage returned no series');
    ...
    try {
        ...
        if (ov && ov.MarketCapitalization)
            marketCap = Number(ov.MarketCapitalization);
        if (ov && ov.PERatio)
            peRatio = Number(ov.PERatio);
    }
    catch (err) {
...
async function enrichFundamentalsFromYahoo(ticker, numbers) {
    try {
        const q = await yahooFinance.quote(ticker);
        if (q) {
            if (q.marketCap)
                numbers.marketCap = q.marketCap;
            if (q.trailingPE !== undefined && q.trailingPE !== null)
                numbers.peRatio = q.trailingPE;
            if (q.currency)
                numbers.currency = q.currency;
            if (q.regularMarketChangePercent !== undefined && q.regularMarketChangePercent !== null)
                numbers.change = q.regularMarketChangePercent;
            if (q.regularMarketPrice !== undefined && q.regularMarketPrice !== null)
                numbers.price = q.regularMarketPrice;
        }
    }
    catch (err) {
...
async function fetchOverviewAndFinancials(ticker, companyName) {
    const overview = {
        name: companyName,
        ticker,
        industry: 'Unknown',
...
const runResearchAgent = async (companyName, treatAsTicker = false) => {
    ...
    try {
        ...
        let searchResult = { quotes: [] };
        if (preferTicker) {
            try {
                const ticker = maybeTicker;
                const cached = await (0, cache_1.getCache)(ticker);
                if (cached) {
                    ...
                    if (ageMs < sixHours) {
                        ...
                        if ((!numbers?.marketCap || numbers.marketCap === 0) || (numbers?.peRatio === null || numbers?.peRatio === undefined)) {
                            ...
                            await (0, cache_1.setCache)(ticker, numbers, chartData);
                        }
                    }
                }
                ...
                if (!chartData || chartData.length === 0) {
                    ...
                    const queryOptions = { period1: sixMonthsAgo, period2: today, interval: '1d' };
                    ...
                    chartData = historical.map((day) => ({ date: day.date.toISOString().split('T')[0], price: day.close }));
    ...
    const searchTool = new duckduckgo_search_1.DuckDuckGoSearch({ maxResults: 5 });
    try {
    ...
    try {
        if (metadata?.ticker && numbers && ((!numbers.marketCap || numbers.marketCap === 0) || (numbers.peRatio === null || numbers.peRatio === undefined))) {
            await enrichFundamentalsFromYahoo(metadata.ticker, numbers);
            if (metadata.ticker)
                await (0, cache_1.setCache)(metadata.ticker, numbers, chartData);
        }
    }
    catch (err) {
    ...
    try {
        if (metadata?.ticker && numbers) {
            const q = await yahooFinance.quote(metadata.ticker);
            if (q) {
                if (q.marketCap)
                    numbers.marketCap = q.marketCap;
                if (q.trailingPE !== undefined && q.trailingPE !== null)
